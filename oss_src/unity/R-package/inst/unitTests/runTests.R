...
if (require("RUnit", quietly = TRUE)) {
  ...
  ## Define tests
  testSuite <-
    defineTestSuite(name = paste(pkg, "unit testing"), dirs = path)
  ...
  checkForAllTests <- function() {
    ...
    Sys.setenv("RunAllDatoCoreTests" = "no")
    return(FALSE)
  }
  ...
  if (Sys.getenv("RunAllDatoCoreTests") == "") {
  ...
  if (interactive()) {
    ...
    if (exists("sframe.unit.test.output.dir", globalenv())) {
      output <- sframe.unit.test.output.dir
    } else {
    ...
    if (is.null(output)) {
      # if it did not work, use parent dir
      output <-
        ".."              # as BDR does not want /tmp to be used
    }
    
    ## Print results
    output.txt  <-
      file.path(output, sprintf("%s-unitTests.txt", pkg))
    output.html <-
      file.path(output, sprintf("%s-unitTests.html", pkg))
    ...
    if ((err$nFail + err$nErr) > 0) {
      data <- Filter(function(x)
        any(sapply(x, function(.)
          .[["kind"]]) %in% c("error","failure")) ,
        tests[[1]]$sourceFileResults)
      err_msg <- sapply(data,
                        function(x) {
                          raw.msg <- paste(sapply(Filter(
                            function(.)
                              .[["kind"]] %in% c("error","failure"), x
                          ), "[[", "msg"),
                          collapse = " // ")
                          raw.msg <-
                            gsub(
      ...
      msg <-
        sprintf(
          "unit test problems: %d failures, %d errors\n%s",
          err$nFail, err$nErr,
