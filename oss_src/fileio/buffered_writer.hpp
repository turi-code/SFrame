 ...
 * std::vector<graphlab::mutex>  sink_mutex; // size N
 *
...
public:
  buffered_writer(OutIterator& out, graphlab::mutex& out_lock,
  ...
  void flush(bool is_locked = false) {
    ...
    std::lock_guard<graphlab::mutex> guard(out_lock, std::adopt_lock);
    for (auto& val : buffer) {
      *out++ = std::move(val);
    }
    buffer.clear();
