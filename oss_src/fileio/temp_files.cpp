 ...
 * object is destroyed. (See \ref graphlab::global_teardown).
 */
...
std::vector<std::string> get_temp_directories() {
  ...
  if(fileio::get_cache_file_locations() == std::string("CHANGEME")) {
    fileio::set_cache_file_locations(graphlab::fileio::get_system_temp_directory());
  }
...
/**
 * Return graphlab-[user_name]
 */
static std::string get_graphlab_temp_directory_prefix() {
  std::string graphlab_name = "graphlab";
  std::string user_name = get_system_user_name();
  if (!user_name.empty()) {
    graphlab_name = graphlab_name + "-" + user_name;
  }
  return graphlab_name;
}
...
static fs::path get_current_process_hdfs_temp_directory() {
  ...
  if (fileio::get_cache_file_hdfs_location() != "") {
    ...
    std::string graphlab_name = get_graphlab_temp_directory_prefix();
    path /= graphlab_name;
  }
  return path;
}
...
void reap_current_process_temp_files() {
  ...
  for (auto fname: get_temp_info().tempfile_history) {
    graphlab::fileio::delete_path_impl(fname);
  }
  for (auto current_temp_dir: get_temp_info().process_temp_directories) {
    ...
    try {
      if (fileio::get_protocol(string_dir) == "hdfs") {
        ...
        graphlab::fileio::delete_path_impl(string_dir);
      } else {
        logstream(LOG_DEBUG) << "Recursive deletion of "
                             << current_temp_dir << std::endl;
        graphlab::fileio::delete_path_recursive(string_dir);
      }
    } catch (...) { }
  }
}
