...
#ifndef GRAPHLAB_SERIALIZE_GL_VECTOR_HPP
#define GRAPHLAB_SERIALIZE_GL_VECTOR_HPP
...
template <typename T> class gl_vector; 
  ...
  namespace archive_detail {
    ...
    template <typename OutArcType, typename ValueType>
    struct gl_vector_serialize_impl<OutArcType, ValueType, true > {
      static void exec(OutArcType& oarc, const gl_vector<ValueType>& vec) {
        oarc << size_t(vec.size());
    ...
    struct gl_vector_deserialize_impl<InArcType, ValueType, false > {
    ...
    template <typename InArcType, typename ValueType>
    struct gl_vector_deserialize_impl<InArcType, ValueType, true > {
      static void exec(InArcType& iarc, gl_vector<ValueType>& vec){
        size_t len;
        iarc >> len;
        vec.clear(); vec.resize(len);
